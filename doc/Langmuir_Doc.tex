...
The electric field may be defined using complex numbers: $E=E_{0}exp(i\omega t)$.
In a typical plasma etching application, $\omega=2\pi f$, where $f=13.56$
MHz. Let's look at the electron momentum equation,
\[
m_{e}n_{e}\frac{\text{\ensuremath{\partial}}}{\partial t}u_{e}=-en_{e}E_{0}exp(i\omega t)-\nabla p_{e}-m_{e}n_{e}v_{coll\_em}u_{e}
\]
...
\item electron velocity follows the same oscillation, $u=u_{0}exp(i\omega t)$.
\end{enumerate}
The electron momentum equation linearizes to
\[
(n_{e}m_{e}i\omega)u_{0}exp(i\omega t)=\text{\textminus}n_{e}eE_{0}exp(i\omega t)\text{\textminus}(n_{e}m_{e}v_{coll\_em})u_{0}exp(i\omega t)
\]
